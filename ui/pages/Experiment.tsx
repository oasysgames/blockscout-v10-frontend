...
const PieChart = ({
  ...
  totalAccumulatedByChain: Array<{ chainName: string; accumulated_amount: number }>;
  bgColor: string;
  textColor: string;
}) => {
...
const Experiment = () => {
  ...
  const handleStartDateInputChange = React.useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    handleStartDateChange(event.target.value);
  }, [ handleStartDateChange ]);
  const handleEndDateInputChange = React.useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    handleEndDateChange(event.target.value);
  }, [ handleEndDateChange ]);
  const handleChainFilterSelectChange = React.useCallback((event: React.ChangeEvent<HTMLSelectElement>) => {
    handleChainFilterChange(event.target.value);
  }, [ handleChainFilterChange ]);
  const formatChartValue = React.useCallback((val: number) => {
    ...
    <>
      ...
      <Box mb={ 6 }>
        <Flex gap={ 4 }>
          ...
          <select
            value={ chainFilter }
            onChange={ handleChainFilterSelectChange }
            style={{
              padding: '8px',
              borderRadius: '4px',
