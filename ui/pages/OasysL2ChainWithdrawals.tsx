...
const OasysL2ChainWithdrawals = () => {
  ...
  const eventType: EventType = 'WITHDRAW';
      ...
      <Skeleton loading={ countersQuery.isPlaceholderData || isLoading } display="flex" flexWrap="wrap">
        { countersQuery.data && (
          <Text lineHeight={{ base: '24px', lg: '32px' }}>
            ...
            and { calculateUsdValue({ amount: countersQuery.data.withdrawal_sum, decimals: 18 }).valueStr } { currencyUnits.ether } has been withdrawn
          </Text>
        ) }
      </Skeleton>
    );
